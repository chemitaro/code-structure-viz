...
| I07-REQ-002 | source acquisition | domain無指定ではpython、sqlalchemy、nextをdeterministic orderで選ぶ。snapshotはone runのpreflight、run-start immutable SourceView、resolved config、output transactionだけを共有し、implicit base、start-HEAD anchor、FileChangeSet、ChangedPathAdmissionGateを使わない。diffはone runのendpoint resolution、working-tree freeze、start-HEAD anchor、metadata-only FileChangeSet、changed-path admissionを共有する。 |
...
| I07-REQ-004 | Artifact/output | 各complete domainと`partial_safe` snapshot domainはown semantic JSONとdomain-specific PlantUMLを生成する。not_applicableはstatus/diagnosticのみ、`payload_unavailable`（diff side failure、global/unsafe failure、entity budget超過を含む）はaffected JSON/PlantUMLを生成しない。aggregateは`code-structure-viz.run-manifest/v1`だけとする。 |
| I07-REQ-005 | failure behavior | 各domainは共通presence truth tableに従う。run-level changed-path overrunはexit 1で全Artifact/final manifestなし、domain-local entity overrunはaffected domain `incomplete_kind: payload_unavailable`・exit 3・affected payloadなしでsibling Artifactとaggregate manifestを保持する。 |
...
| I07-REQ-008 | stdout contract | `--stdout SELECTOR`を高々1回のclosed selectorとして検証し、available exact bytes、unavailable result、selectorなしsummary、stderr diagnostics、exit 2 no-publicationを提供する。 |
...
domain無指定ではpython、sqlalchemy、nextをdeterministic orderで選ぶ。snapshotはone runのpreflight、run-start immutable SourceView、resolved config、output transactionだけを共有し、implicit base、start-HEAD anchor、FileChangeSet、ChangedPathAdmissionGateを使わない。diffはone runのendpoint resolution、working-tree freeze、start-HEAD anchor、metadata-only FileChangeSet、changed-path admissionを共有する。
### I07-REQ-003
...
各complete domainと`partial_safe` snapshot domainはown semantic JSONとdomain-specific PlantUMLを生成する。not_applicableはstatus/diagnosticのみ、`payload_unavailable`（diff side failure、global/unsafe failure、entity budget超過を含む）はaffected JSON/PlantUMLを生成しない。aggregateは`code-structure-viz.run-manifest/v1`だけとする。
### I07-REQ-005

各domainは共通presence truth tableに従う。run-level changed-path overrunはexit 1で全Artifact/final manifestなし、domain-local entity overrunはaffected domain `incomplete_kind: payload_unavailable`・exit 3・affected payloadなしでsibling Artifactとaggregate manifestを保持する。
### I07-REQ-006
...
code-structure-viz diff --repo . --domain python --format semantic-json --stdout python:semantic-json --output-dir /tmp/csv-python-stdout
```

### source acquisition contract

- domain無指定ならpython、sqlalchemy、nextをdeterministic orderで実行する。snapshotも同じdefault、明示domainで絞り込める。
- snapshotはcore preflight、run-start immutable SourceView、resolved config、output transactionだけをone runで共有する。diffだけがendpoint resolution、working-tree freeze、start-HEAD anchor、metadata-only FileChangeSet、changed-path admissionをone runで共有する。各adapterはどちらのuse caseでもdomain-owned source selectionとsemantic modelを保持する。
...
- diffの`FileChangeSet` hunkはmetadataだけを持つ。許可項目はrepository-relative old/new path、file status、old/new start line、old/new line count、ordinal、これらのcanonical tupleから生成したcontent-independent SHA-256 `hunk_id`である。raw patch/context/added/deleted lines、source body、comment、literal、secret、absolute pathをmodel、JSON、PlantUML、manifest、diagnostic、logへ保持・公開しない。
- diffのimplicit changed-path budgetはdomain比較前のrun-level admission gateでdefault 1,000。overrideなしでactual countが超過したrunはfatal analysis/environment、exit 1、safe machine-readable diagnosticのみとし、semantic JSON、PlantUML、final run manifestを公開しない。positive integerの`--max-changed-paths N`は通常処理を許可し、manifestへrequested/resolved/count/config sourceを記録する。invalid overrideはexit 2。
...
- `not_applicable` domainはstatus/diagnosticのみ。`partial_safe` snapshot domainはstatus `incomplete`をsemantic JSON、PlantUML、manifestへ明示してsafe subset payloadを公開する。diff side acquisition/analysis failure、global/unsafe failure、entity budget超過は`payload_unavailable`としてaffected semantic JSON/PlantUMLを公開しない。
...
- adapter exceptionはcore crashへ伝播させずdomain diagnosticへ正規化する。protocol corruption、global schema failure、security invariant violation、unsafe pathは`payload_unavailable`とし、affected payloadを公開しない。isolated snapshot failureでsafe subset、coverage、redaction、entity-budget passを証明できる場合だけ`partial_safe`を許す。
- domain presenceは上記truth tableに従い、before-only/after-onlyをcomplete全removed/added、both-absentをnot_applicable、diff side failureを`incomplete_kind: payload_unavailable`とする。
...
| I07-AC-009 | changed-path overrunはexit 1・final manifestなし、entity overrunはaffected domain `payload_unavailable`・exit 3・affected payloadなし・sibling/manifest保持、valid overridesはrequested/resolved/countを記録する。 | I07-AT-009 |
...
| I07-AC-012 | stdout selectorのvalid/invalid/duplicate/domain/format、exact-byte（`partial_safe`を含む）、not_applicable/payload_unavailable/fatal/interrupt result、selectorなしsummaryをtable-drivenに満たす。 | I07-AT-012 |
| I07-AC-013 | all-domain snapshotはimplicit base/start-HEAD/FileChangeSet/ChangedPathAdmissionGateを使わず、base不能と1,001 changed pathsの影響を受けない。diff-only option併用はexit 2、Artifactなしとなる。 | I07-AT-013 |
| I07-AC-014 | all-domain snapshotのisolated local failureは`partial_safe` JSON+PlantUML+manifestを保持し、global/unsafe/entity-overrunまたはdiff side failureは`payload_unavailable`としてaffected payloadを除外し、健全siblingを保持する。 | I07-AT-014 |

- **I07-AC-001〜I07-AC-014 がすべて満たされ、planned test command が clean checkout で成功すること。**
