...
- indexの `skip-worktree` と `assume-unchanged` flag は `git ls-files -v --cached` から同じraw identityを照合して取得する。skip-worktree pathが作業木に欠落していても `sparse-unavailable` として `payload_unavailable`（exit 3）へ縮退し、`D` やcanonical empty side、index blobの再構築へ変換しない。通常tracked pathの欠落だけを実削除 `D` とする。
