...
per-worktree Git directory自身またはその `worktrees/` descendantに限定したcommon Git directoryを解決する。
common directoryのbinding identityと `info/exclude` をprofileへ含め、common directory直下のregular
`info/exclude`を実際のignore authorityとしてdigest化する。開始・終了のbinding、`core.ignoreCase`、common
exclude、untracked observationが不一致なら `CSV-SOURCE-001`、初期の解決不能は `CSV-DIFF-003` とする。
