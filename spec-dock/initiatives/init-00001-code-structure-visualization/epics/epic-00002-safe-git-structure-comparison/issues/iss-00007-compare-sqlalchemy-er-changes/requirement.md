...
| I04-REQ-002 | SQLAlchemy snapshot の既存 ID を基準に table・row・relation を比較し、`added` / `removed` / `modified` を safe before/after value とともに出力する。ID が変わった定義を rename/move と推測せず `removed + added` とする。 |
...
- 同一 ID の safe semantic value が変わった場合は `modified`。
...
| ID | 完了条件 |
| --- | --- |
| I04-AC-001 | complete before/after で table・Issue #6 row・relation の added/removed/modified と impact context が正しく出力され、ID変更はremoved+addedになる。 |
