"""Frozen Unicode 15.0.0 NFC profile used by the Issue #8 contract.

The compressed payload is checked in and contains every non-zero canonical
combining class, every canonical decomposition, and every non-Hangul canonical
composition pair from Unicode 15.0.0. Runtime normalization never consults
the host Unicode database. Hangul algorithmic decomposition/composition is
implemented directly from the Unicode Standard.

The payload is independently matched against UnicodeData.txt and
DerivedNormalizationProps.txt from Unicode 15.0.0. Source digests and
reproduction rules are in docs/contracts/next-unicode-nfc-v1.md. The data's
copyright/permission notice is tests/fixtures/unicode-license.txt.
``NFC_TABLE_DIGEST`` binds the tables; ``FULL_SCALAR_KAT_DIGEST`` covers
individual scalars, while the official NormalizationTest vectors exercise
multi-scalar ordering and composition blocking independently.
"""
